...
async function getSignedInUser(
  ...
  if (error || !data.user) {
    throw new Error(
      "Please sign in before cancelling your subscription.",
    );
  }
...
async function getStoredSubscription(
  ...
  return data as StoredSubscriptionRecord | null;
}

async function getBillingRecord(userId: string) {
  const supabase = getSupabaseAdmin();
...
function validateSubscriptionOwnership({
  ...
  if (
    ...
    throw new Error(
      "The Revolut subscription reference does not match this account.",
    );
  }
...
async function cancelRevolutSubscription({
  secretKey,
  subscriptionId,
}: {
  secretKey: string;
  ...
  throw new Error(
    cleanText(resultObject?.message) ||
      cleanText(resultObject?.error) ||
      "Revolut cancellation failed. Please try again or contact support.",
  );
}
...
export async function POST(
  ...
  try {
    ...
    const [access, billing] =
      await Promise.all([
        getStoredSubscription(user.id),
        getBillingRecord(user.id),
      ]);
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "This account does not have a Revolut subscription to cancel.",
        },
        { status: 409 },
      );
    }
    ...
    const localBillingStatus = cleanText(
      billing?.status,
    ).toLowerCase();

    const localSubscriptionStatus = cleanText(
      access.subscription_status,
    ).toLowerCase();

    if (
      !validLocalStatuses.includes(
        localBillingStatus,
      ) &&
      !validLocalStatuses.includes(
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "There is no active or pending Revolut subscription to cancel.",
        },
        { status: 409 },
      );
    }
    ...
    await cancelRevolutSubscription({
      secretKey,
      subscriptionId,
    });
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Could not cancel the Revolut subscription.";

    console.error(
      "Cancel Revolut subscription error:",
      message,
    );
    ...
    const status =
      ...
      lowerMessage.includes("auth token")
        ...
        : lowerMessage.includes(
          ...
          ? 403
          : lowerMessage.includes(
                "no revolut subscription",
              ) ||
              lowerMessage.includes(
                "no active or pending",
              )
            ? 409
            : 500;
