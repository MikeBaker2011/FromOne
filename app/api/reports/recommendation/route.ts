...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: metrics, error: metricsError } = await supabase
      .from('post_metrics')
      .select(
        'id,campaign_post_id,user_id,platform,provider_post_id,likes,comments,shares,reach,impressions,saves,engagement,synced_at',
      )
      .eq('user_id', user.id)
    ...
    const campaignPostIds = Array.from(
      new Set(
        metricRows
          .map((row: any) => cleanText(row.campaign_post_id))
          .filter(Boolean),
      ),
    );
    ...
    if (campaignPostIds.length) {
      const { data: postRows, error: postsError } = await supabase
        .from('campaign_posts')
        .select('id,title,platform,status,publish_status,created_at,published_at')
        .eq('user_id', user.id)
        .in('id', campaignPostIds);

      if (postsError) {
        throw new Error(`Could not load matching campaign posts: ${postsError.message}`);
      }

      postsById = new Map(
        (postRows || []).map((post: any) => [cleanText(post.id), post]),
      );
    }

    const rows = metricRows.map((row: any) => ({
      ...row,
      campaign_posts: postsById.get(cleanText(row.campaign_post_id)) || null,
    }));
