...
async function getRecentUsers(limit = 30) {
  const supabase = getSupabaseAdmin();
  const { data, error } = await supabase.auth.admin.listUsers({
    page: 1,
    perPage: Math.max(limit, 30),
  });
  ...
  return (data?.users || [])
    .sort((firstUser, secondUser) => {
      const firstTime = firstUser.created_at ? new Date(firstUser.created_at).getTime() : 0;
      const secondTime = secondUser.created_at ? new Date(secondUser.created_at).getTime() : 0;
...
async function getRecentBugReports() {
  ...
  if (error) {
    // Keep the admin dashboard usable even if the support table has not been created yet.
    if (error.code === 'PGRST205' || String(error.message || '').includes('bug_reports')) {
      return [];
    }
  ...
  const uniqueUserIds = Array.from(
    new Set(reports.map((report: any) => cleanText(report.user_id)).filter(Boolean)),
  );
  ...
  return reports.map((report: any) => ({
    ...report,
    user_email: report.user_id ? emailByUserId.get(report.user_id) || null : null,
  }));
}
